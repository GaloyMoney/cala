...
/// The caught-up barrier: after `await_ec_caught_up` returns `Ok`, every
/// transaction committed before the call is folded into EC balances —
/// settled *and* cumulative-effective (same commit) — and readable
/// immediately, with **no** further polling. A second fence after more
/// posts renews the guarantee (the semantics are self-renewing, not
/// one-shot).
#[tokio::test]
async fn await_ec_caught_up_fences_backlog_and_renews() -> anyhow::Result<()> {
    let usd: Currency = "USD".parse().unwrap();
    let pool = helpers::init_isolated_pool().await?;
    let (fixture, mut jobs) = setup(pool, helpers::test_journal_with_effective_balances()).await?;
    ...
    fixture
        .cala
        .await_ec_caught_up(std::time::Duration::from_secs(60))
        .await?;
...
#[tokio::test]
async fn await_ec_caught_up_times_out_when_rollup_is_stalled() -> anyhow::Result<()> {
    let pool = helpers::init_isolated_pool().await?;
    let (fixture, _jobs) = setup(pool, helpers::test_journal()).await?;
    ...
    match fixture
        .cala
        .await_ec_caught_up(std::time::Duration::ZERO)
        .await
    {
    ...
    match fixture.cala.await_ec_caught_up(timeout).await {
